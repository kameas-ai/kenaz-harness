...
# production Open path by upgrade_path_test.go. Its CREATE TABLE
# session_messages line necessarily names kind / move_index /
# turn_span_id / model_tool_args, because that is the schema being
# frozen. It is captured output, cannot execute, and is regenerated by
# scripts/ci/upgrade-snapshot.sh rather than hand-edited (and
# check-upgrade-snapshots-locked.sh forbids editing it). Scoped to .sql
# under a testdata/ directory ONLY: a .go file under testdata/ is still
# scanned, since that could genuinely write.
