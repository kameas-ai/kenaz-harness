...
| I14 | `check-broker-topic-consumers.sh` | `i14-unconsumed-broker-topics.txt` | `Topic* = "…"` broker-topic const with no frontend subscriber, no Go subscriber, and no `passthroughTopics` entry — **added 2026-08-18, empty**. Multi-pass (Go + frontend), same discipline as `check-output-ports.sh`. See "Declined gate" below for why this shipped instead of the gate `docs/dead-code-audit-2026-08-16.md` §5 originally asked for. |
