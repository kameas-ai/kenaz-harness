...
// the same engine surface a production chassis would: real Engine
// constructed via NewEngine, in-memory message store + canned-response
...
//
// We deliberately reuse the fakes declared in engine_test.go
...
func buildLargeFixture(totalMessages int) []Message {
	out := make([]Message, totalMessages)
...
// the success.
//
// Engine-scope translation: instead of running 100 chat turns through
...
func TestIntegration_ThresholdTier_AggressiveFiresAt60Pct(t *testing.T) {
	...
	aggressive := Tier(AggressivenessAggressive)
...
func TestIntegration_MaximalTier_RollsEveryCall(t *testing.T) {
	...
	maximal := Tier(AggressivenessMaximal)
	if maximal.Mode != ModeRolling {
		t.Fatalf("maximal tier mode drift: got %v, want ModeRolling", maximal.Mode)
	}
...
// item 4 at the engine layer: when aggressiveness=off, the chat runner
// is the seam that decides not to invoke Compact / RollingSummarize at
// all (Tier(off).Mode == ModeNone). The chat runner's actual short-circuit
// is unit-tested in WP08's chat_runner_test.go; here we assert the
// policy invariant the runner reads.
//
// The integration assertion here is "Tier(off) returns ModeNone with
// zero TriggerPct/SummarizePct" — a runner reading these values has no
// path that schedules a compaction call. If a regression flipped any
// of those numerics, the runner's branching logic in
// runPreSendCompaction would silently start running compactions.
func TestIntegration_OffTier_EngineIsBypassed(t *testing.T) {
	off := Tier(AggressivenessOff)
	if off.Mode != ModeNone {
		t.Fatalf("off tier mode drift: got %v, want ModeNone", off.Mode)
	}
	if off.TriggerPct != 0 {
...
func TestIntegration_AllFiveTiers_HaveDistinctBehavior(t *testing.T) {
	...
	t.Run("off", func(t *testing.T) {
		...
		_, err := eng.Compact(context.Background(), "s", ProviderProfileRef{}, Tier(AggressivenessOff).SummarizePct)
		if err != nil {
			t.Fatalf("Compact: %v", err)
		}
	...
	for _, tc := range []struct {
		name       string
		tier       CompactionAggressiveness
		wantArchived int
	}{
		{"conservative", AggressivenessConservative, 20},
		{"balanced", AggressivenessBalanced, 30},
		{"aggressive", AggressivenessAggressive, 40},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			summarizePct := Tier(tc.tier).SummarizePct
