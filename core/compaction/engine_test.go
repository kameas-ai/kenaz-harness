...
// engine_test.go drives the threshold-mode engine through fakes for
...
func (s *fakeStore) ListActiveMessages(_ context.Context, _ string) ([]Message, error) {
	if s.listErr != nil {
...
func (s *fakeStore) ApplyCompaction(_ context.Context, sessionID string, summary Message, originalIDs []string, archivedAt time.Time) error {
...
func build100MessageFixture() []Message {
	out := make([]Message, 100)
	for i := 0; i < 100; i++ {
		role := "user"
		if i%2 == 1 {
			role = "assistant"
...
// newTestEngine wraps NewEngine with sensible test defaults (deterministic
// clock, no recent-window clamp by default — tests opt into it).
func newTestEngine(t *testing.T, store MessageStore, llm LLMCaller, caps CapabilityLookup, em AuditEmitter, recentWindow int) Engine {
	t.Helper()
...
func TestNewEngine_RequiresDeps(t *testing.T) {
	cases := []struct {
		...
		{"missing store", EngineConfig{LLM: &fakeLLM{}, Capabilities: fakeCapabilities{}}},
		{"missing llm", EngineConfig{Store: &fakeStore{}, Capabilities: fakeCapabilities{}}},
		{"missing capabilities", EngineConfig{Store: &fakeStore{}, LLM: &fakeLLM{}}},
	}
