...
// scheduler_test.go drives the periodic Scheduler through a fake
// runOnce closure. The tests set short intervals (50ms) for the
// tick-based assertions and rely on WithClock + SeedLastRun for the
...
func TestScheduler_FirstRunAtStartupWhenLastRunZero(t *testing.T) {
	...
	s := NewScheduler(runOnce, WithInterval(24*time.Hour))
...
func TestScheduler_NoImmediateRunWhenRecent(t *testing.T) {
	...
	s := NewScheduler(runOnce, WithInterval(24*time.Hour), WithClock(clock))
...
func TestScheduler_StopIsIdempotent(t *testing.T) {
	s := NewScheduler(func(ctx context.Context) (int, error) { return 0, nil },
