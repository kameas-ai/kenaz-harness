...
func startPausedApprovalRun(t *testing.T, a *graphview.Impl) (runID string, st graphview.RunStatus) {
	...
	if err := a.SaveGraph(ctx, graphview.GraphSpec{ID: "resolve_approval_rpc", YAML: resolveApprovalGraphYAML}); err != nil {
...
nodes:
  - id: q
    ...
    attrs:
      question: "Name?"
`
	if err := a.SaveGraph(ctx, graphview.GraphSpec{ID: "ask_cross_kind", YAML: askYAML}); err != nil {
		t.Fatalf("SaveGraph: %v", err)
	}
...
func TestResolveApproval_CedarDenyBlocksResolution(t *testing.T) {
	...
	if calls := gate.calls(); len(calls) == 0 || calls[0] != policycedar.ActionApprovalResolve {
		t.Errorf("gate.calls() = %v, want at least one %q", calls, policycedar.ActionApprovalResolve)
	}
...
`
		ctx := context.Background()
		if err := a.SaveGraph(ctx, graphview.GraphSpec{ID: "auto_window_zero", YAML: yaml}); err != nil {
			t.Fatalf("SaveGraph: %v", err)
		}
...
`
		ctx := context.Background()
		if err := a.SaveGraph(ctx, graphview.GraphSpec{ID: "auto_window_positive", YAML: yaml}); err != nil {
			t.Fatalf("SaveGraph: %v", err)
		}
