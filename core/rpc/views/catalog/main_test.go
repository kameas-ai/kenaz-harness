...
//
// impl_test.go calls fleet.SaveTokens (via a wired keyring/CedarPublish
// test path), which on darwin talks to the real Keychain. That is fine
// under a developer's own HOME — the entry already exists and the call
// returns in milliseconds. Under a REDIRECTED HOME it is not: the
