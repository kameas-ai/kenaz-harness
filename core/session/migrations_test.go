...
func TestMigrations_RegisterAndApply(t *testing.T) {
	...
	// Ledger rows: 2 storage bootstrap + 18 sessions migrations
	...
	// 0314 session_usage_columns + 0316 autonomy_columns +
	// 0317 streaming_resume_columns + 0318 kind) = 20 applied entries
	// (2 chassis bootstrap + 18 sessions migrations).
	if got := len(db.ledger); got != 20 {
		t.Fatalf("ledger size = %d, want 20", got)
	}
	wantVersions := []int{1, 2, 300, 301, 302, 303, 304, 305, 306, 307, 308, 309, 310, 311, 312, 313, 314, 316, 317, 318}
