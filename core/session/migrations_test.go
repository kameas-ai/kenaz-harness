...
func TestMigrations_RegisterAndApply(t *testing.T) {
	...
	// 0309 agent_graph_events + 0310 compaction +
	// 0317 streaming_resume_columns) = 14 applied entries.
	if got := len(db.ledger); got != 14 {
		t.Fatalf("ledger size = %d, want 14", got)
	}
	wantVersions := []int{1, 2, 300, 301, 302, 303, 304, 305, 306, 307, 308, 309, 310, 317}
