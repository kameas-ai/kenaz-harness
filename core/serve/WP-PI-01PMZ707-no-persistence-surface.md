...
> **Why this is a `.md` and not a `_test.go`.** It shipped as
> `core/serve/wp_pi_persistence_integrity_01pmz707_test.go` containing 125 lines
> of comment and **zero test functions** — `grep -c '^func '` returned 0. The
> N/A argument below is sound and was verified, but a file named `_test.go`
> that adds no test is counted as coverage by anyone reading the filename,
> which is the same "absence of evidence rendered as evidence of absence"
> failure the WP-PI template exists to prevent. Renamed after the independent
> review of PR #304 (finding F4). The content is unchanged.
