...
// `kenaz lock --resolve-conflicts` subcommand. The full implementation
...
//
//	$ kenaz lock --resolve-conflicts
//	error: feature deferred to v1.x. To unblock now, regenerate the
//	  lockfile with `kenaz lock --regenerate`.
//
// See plan §8 R3 for the rationale.
func ResolveConflicts(_ ...*Lockfile) error {
	return fmt.Errorf("%w: kenaz lock --resolve-conflicts is planned for v1.x", bundle.ErrNotImplementedV1x)
}
