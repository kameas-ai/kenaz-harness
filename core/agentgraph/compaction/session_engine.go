...
//
// session_engine.go houses the threshold-mode summarize-then-replace compaction
...
// SessionEngine performs threshold-mode session compaction.
//
// Concurrency: callers MUST ensure a single SessionEngine call per session at
...
// SessionEngineConfig is the dependency-injection envelope. See the field
...
// SessionMessageStore is the persistence boundary. Implementations live in
...
type SessionMessageStore interface {
	...
	ListActiveMessages(ctx context.Context, sessionID string) ([]SessionMessage, error)
	...
	ApplyCompaction(ctx context.Context, sessionID string, summary SessionMessage,
...
// counts. The default implementation in NewSessionEngine wraps
...
// SessionMessage is the engine's internal message shape. It carries the
...
// engine is the default SessionEngine implementation. Constructed via NewSessionEngine.
type engine struct {
	store        SessionMessageStore
	llm          LLMCaller
	capabilities CapabilityLookup
...
// SessionEngineConfig.RecentWindow is nil. Callers wiring the real settings
...
// NewSessionEngine constructs the threshold-mode engine. Returns an error if
// required dependencies are missing. Optional fields (Now,
// RecentWindow, Tokenizer) get sensible defaults.
func NewSessionEngine(cfg SessionEngineConfig) (SessionEngine, error) {
	if cfg.Store == nil {
		return nil, fmt.Errorf("compaction: SessionEngineConfig.Store is required")
	}
	if cfg.LLM == nil {
		return nil, fmt.Errorf("compaction: SessionEngineConfig.LLM is required")
	}
	if cfg.Capabilities == nil {
		return nil, fmt.Errorf("compaction: SessionEngineConfig.Capabilities is required")
	}
...
func (e *engine) Compact(ctx context.Context, sessionID string, model ProviderProfileRef,
	...
	// Step 3: Tool-pair clamp. Pure helper in session_snap.go.
	boundary = snapBoundaryForToolPairs(messages, boundary)

	// Step 4: Recent-window clamp. Pure helper in session_snap.go.
	rw := e.recentWindow()
...
func renderTranscript(span []SessionMessage) string {
...
// directly via SessionEngineConfig.Tokenizer.
type defaultTokenizer struct{}

// Count translates the engine's TokenizerMessage shape into the
// tokenizer package's SessionMessage shape and delegates. The wrapper exists
