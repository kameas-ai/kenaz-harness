...
// session_rolling_test.go drives the maximal-mode rolling-summary path
// (RollingSummarize) through the same fakes WP03's session_engine_test.go
// uses. We DO NOT redefine those fakes — the tests below reuse
// fakeStore / fakeLLM / fakeCapabilities / fakeAudit / fakeTokenizer
// from session_engine_test.go since both files compile into the same test
// binary.
//
...
func buildPairsFixture(numPairs int) []SessionMessage {
	out := make([]SessionMessage, 0, numPairs*2)
...
func TestFindRecentWindowStart_Direct(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"empty, window 4", []SessionMessage{}, 4, 0},
...
func TestRollingSummarize_SubsequentRoll(t *testing.T) {
	...
	active := append([]SessionMessage{prevRolling}, recent...)
