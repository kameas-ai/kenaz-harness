...
//
// session_rolling.go houses the maximal-mode rolling-summary path (plan §2.5).
// Where Compact (session_engine.go) is a percent-of-tokens threshold flow that
...
//
// The path reuses the WP03 collaborators verbatim — same SessionMessageStore
...
// from the threshold-mode prompt (session_engine.go) for the rolling-summary
...
func (e *engine) RollingSummarize(ctx context.Context, sessionID string,
	...
	// summary.
	rolledTurns := make([]SessionMessage, 0, tailStart)
...
func findRecentWindowStart(messages []SessionMessage, recentWindow int) int {
	if recentWindow <= 0 {
		return len(messages)
	}
	if len(messages) == 0 {
...
func findPreviousRollingSummary(messages []SessionMessage, tailStart int) (string, int, string) {
...
// renderTranscript in session_engine.go.
func renderRollingPile(prevSummaryContent string, rolledTurns []SessionMessage) string {
...
func (e *engine) tokenizePile(prevSummaryContent string, rolledTurns []SessionMessage) int {
...
// mode mirror of emitNoOpAudit (session_engine.go). Centralized so the no-op
