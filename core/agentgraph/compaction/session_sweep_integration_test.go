...
// session_sweep_integration_test.go drives the multi-cycle sweep scheduler
// against a small in-memory SweepStore that holds archived_at
// timestamps + compacted_into_id flags, plus the compaction.WithSweepClock
// option for deterministic time-warp.
//
// These cover plan §4 acceptance-smoke item 5 (date-warp via WithSweepClock)
// plus the scheduler's HARNESS_COMPACTION=off short-circuit and
// goroutine-exit cleanliness.
//
// We deliberately reuse fakeSweepStore from session_sweep_test.go where it
...
func TestIntegration_Scheduler_MultiCycleSweep(t *testing.T) {
	...
	// see session_sweep.go RunSweep's documented contract).
...
// SweepScheduler's tick loop over a short interval against the in-memory
...
// TestIntegration_Scheduler_StopIsClean asserts the SweepScheduler's Stop()
...
func TestIntegration_Scheduler_StopIsClean(t *testing.T) {
	...
	s := NewSweepScheduler(runOnce, WithSweepInterval(20*time.Millisecond))
	s.SeedLastRun(time.Now())
	...
	case <-time.After(2 * time.Second):
		t.Fatalf("SweepScheduler.Stop did not return within 2s; goroutine likely leaked")
	}
...
func TestIntegration_Scheduler_RunOnceManualTrigger(t *testing.T) {
	...
	s := NewSweepScheduler(runOnce,
		WithSweepClock(func() time.Time { return now }),
	)
