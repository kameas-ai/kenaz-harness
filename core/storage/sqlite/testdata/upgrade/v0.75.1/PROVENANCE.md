# Provenance: `testdata/upgrade/v0.75.1/dump.sql`

**Tag**: `v0.75.1`, cut by `tag-on-merge.yml` from **#317, fix(webfetch): pin the dial to the address that was validated**.

## How this snapshot was produced

```bash
bash scripts/ci/upgrade-snapshot.sh v0.75.1
```

`replay mode: v0.75.0 -> v0.75.1` — detached worktree at the tag, `v0.75.0/dump.sql`
...
**Byte-identical to `v0.75.0/dump.sql`**, and verified rather than assumed:

```
$ cmp v0.75.0/dump.sql v0.75.1/dump.sql          -> 0
$ git diff --stat v0.75.0..v0.75.1 -- '*migration*'  -> (empty)
```
