# Provenance: `testdata/upgrade/v0.73.3/dump.sql`

**Tag**: `v0.73.3` (commit `0f62a5ff`).
...
```bash
bash scripts/ci/upgrade-snapshot.sh v0.73.3
```

which reported `replay mode: v0.73.2 -> v0.73.3`, created a detached `git worktree` at
`refs/tags/v0.73.3` (`HEAD is now at 0f62a5ff`), materialised `v0.73.2/dump.sql` into a
...
`v0.73.3` was cut automatically by `tag-on-merge.yml` when a `fix:`-prefixed PR
squash-merged:

    fix(agents): bundled profile models are validated before a paid request (#316)
...
## Schema review for v0.73.3

**`dump.sql` is byte-identical to `v0.73.2/dump.sql`.**

```
$ cmp core/storage/sqlite/testdata/upgrade/v0.73.2/dump.sql \
      core/storage/sqlite/testdata/upgrade/v0.73.3/dump.sql
$ echo $?
0
```
...
```
$ git diff --stat v0.73.2..v0.73.3 -- '*migration*'
(empty)
```

The tag registered no migration, so replaying `v0.73.2`'s database under `v0.73.3`'s
