...
/**
 * ToolPermissionsPanel — Settings panel for tool grants.
 */
...
async function resetAll() {
  if (!window.confirm('Revoke ALL tool grants? This will re-prompt on next use of affected tools.')) {
    return;
  }
  resetting.value = true;
  resetError.value = null;
  try {
    const all: PermissionGrant[] = await client.permissions.listGrants('tool');
    for (const g of all) {
...
<template>
  <section data-testid="tool-permissions-panel">
    <div class="flex items-center justify-between">
      ...
      <button
        ...
        :disabled="resetting"
        data-testid="tool-permissions-reset-all"
        @click="resetAll"
      >
        Reset all
      </button>
    ...
    <p class="mt-1 font-ui text-[11px] text-ink-dim">
      Saved "Allow always" grants for MCP tool calls. Revoking re-prompts on next tool call.
    </p>
    <div
      v-if="resetError"
    ...
    <div class="mt-2">
      <PermissionList ref="listRef" family="tool" />
    </div>
  </section>
</template>
