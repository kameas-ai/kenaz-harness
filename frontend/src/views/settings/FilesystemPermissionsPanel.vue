...
/**
 * FilesystemPermissionsPanel — Settings panel for filesystem grants.
 */
...
async function resetAll() {
  if (!window.confirm('Revoke ALL filesystem grants? This will re-prompt on future filesystem operations.')) {
    return;
  }
  resetting.value = true;
  resetError.value = null;
  try {
    const all: PermissionGrant[] = await client.permissions.listGrants('fs');
    for (const g of all) {
...
<template>
  <section data-testid="filesystem-permissions-panel">
    <div class="flex items-center justify-between">
      ...
      <button
        ...
        :disabled="resetting"
        data-testid="filesystem-permissions-reset-all"
        @click="resetAll"
      >
        Reset all
      </button>
    ...
    <p class="mt-1 font-ui text-[11px] text-ink-dim">
      Saved "Allow always" grants for filesystem operations. Revoking re-prompts on next call.
    </p>
    <div
      v-if="resetError"
    ...
    <div class="mt-2">
      <PermissionList ref="listRef" family="fs" />
    </div>
  </section>
</template>
